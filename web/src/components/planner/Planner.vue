...
export default defineComponent({
  ...
  setup(props) {
    let factories = reactive<Factory[]>(JSON.parse(localStorage.getItem('factoryGroups') || '[]') as Factory[]);
    let worldRawResources = reactive<{ [key: string]: WorldRawResource }>({});
    let dependencies = ref<FactoryDependency[]>(JSON.parse(localStorage.getItem('factoryDependencies') || '[]'));
    const drawer = ref(false);
    let helpText = ref(localStorage.getItem('helpText') === 'true');
  ...
  methods: {
    ...
    // isOutputSelected(output, currentInputIndex, group: Factory) {
