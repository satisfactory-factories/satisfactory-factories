...
export const calculateDependencies = (factories): FactoryDependency => {
  ...
  Object.keys(newDependencies).forEach(factoryDepId => {
    const facDependency = newDependencies[factoryDepId]

    Object.keys(facDependency.requestedBy).forEach(factoryId => {
      const depFactory = facDependency.requestedBy[factoryId]
      depFactory.forEach(request => {
        const part = request.part

        if (!facDependency.metrics[part]) {
          facDependency.metrics[part] = {
            part,
            request: 0,
            supply: 0,
        ...
        facDependency.metrics[part].request += request.amount
      })
    })

    // Calculate the supply and whether the request is satisfied
    const thisFactory = factories.find(fac => fac.id === parseInt(factoryDepId))

    Object.keys(thisFactory.surplus).forEach(product => {
      const amount = thisFactory.surplus[product] // Get the amount of the product from the surplus object

      if (facDependency.metrics[product]) {
        facDependency.metrics[product].supply = amount
        facDependency.metrics[product].isRequestSatisfied = amount >= facDependency.metrics[product].request
      }
    })
  })
